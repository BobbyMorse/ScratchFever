"""
Indiana (Hoosier) winners scraper.

IN Lottery's winners page is paginated HTML, 50 cards per page at:
  https://hoosierlottery.com/winners/?page=N
Each card: event-card-date (Month D, YYYY), winner-prize ("won a $X Prize playing"),
a bold span with the game name, event-winner-store ("at STORE NAME"),
event-winner-city ("in CITY!").
"""
from __future__ import annotations
import datetime as dt
import logging
import re
from backend.scraper.winners.base import WinnersScraper, is_draw_game

logger = logging.getLogger(__name__)

URL = "https://hoosierlottery.com/winners/"

CARD_RE = re.compile(
    r'event-card-date[^>]*>\s*([^<]+?)\s*</span>'
    r'[\s\S]{0,2500}?'
    r'winner-prize"[^>]*>\s*won\s+a\s+\$([\d,.]+)\s+Prize\s+playing\s*</span>'
    r'[\s\S]{0,500}?'
    r'<span[^>]+font-weight-bold[^>]*>\s*([^<]+?)\s*</span>'
    r'[\s\S]{0,500}?'
    r'event-winner-store"[^>]*>\s*at\s+([^<]+?)\s*</span>'
    r'[\s\S]{0,500}?'
    r'event-winner-city"[^>]*>\s*in\s+([^<!]+?)!?\s*</span>',
    re.IGNORECASE,
)


class IndianaWinnersScraper(WinnersScraper):
    state_code = "IN"
    state_name = "Indiana"
    min_prize = 10000.0

    def scrape(self, days: int = 14) -> list[dict]:
        cutoff = dt.date.today() - dt.timedelta(days=days)
        out: list[dict] = []
        seen: set[str] = set()
        page = 1
        while page < 500:
            try:
                resp = self.get(URL, params={"page": page})
            except Exception as e:
                logger.warning("IN page %d failed: %s", page, e)
                break
            matches = list(CARD_RE.finditer(resp.text))
            if not matches:
                break
            stale = 0
            for m in matches:
                norm = self._normalize(m)
                if not norm:
                    continue
                if norm["claim_date"] and norm["claim_date"] < cutoff:
                    stale += 1
                    continue
                if norm["source_id"] in seen:
                    continue
                seen.add(norm["source_id"])
                out.append(norm)
            if stale == len(matches) and stale > 0:
                break
            page += 1
        return out

    def _normalize(self, m) -> dict | None:
        date_raw, prize_raw, game, retailer, city = m.groups()
        try:
            prize = float(prize_raw.replace(",", ""))
        except ValueError:
            return None
        if prize < self.min_prize:
            return None
        game = game.strip()
        if not game or is_draw_game(self.state_code, game):
            return None
        claim_date = None
        for fmt in ("%B %d, %Y", "%b %d, %Y"):
            try:
                claim_date = dt.datetime.strptime(date_raw.strip(), fmt).date()
                break
            except ValueError:
                pass
        retailer = retailer.strip() or None
        city = city.strip().title() or None
        sid_parts = [
            claim_date.isoformat() if claim_date else "",
            retailer or "", city or "", game, f"{int(prize)}",
        ]
        source_id = "|".join(sid_parts)
        return {
            "source_id": source_id,
            "source_game_id": None,
            "source_game_name": game,
            "prize_amount": prize,
            "claim_date": claim_date,
            "retailer_name": retailer,
            "retailer_city": city,
            "retailer_address": None,
            "retailer_zip": None,
            "winner_city": None,
            "retailer_lat": None,
            "retailer_lng": None,
            "source_url": "https://hoosierlottery.com/winners/",
        }
