"""
GA retailer loader — reads from Supabase ga_retailers table.
"""
...
CHAIN_KEYWORDS = [
    ...
    "INGLES ", "PIGGLY WIGGLY", "HARVEY'S", "SEARS", "KMART",
    "COSTCO", "SAM'S CLUB", "WHOLE FOODS", "TRADER JOE",
    "RACETRAC", "RACETRACK", "FLASH FOODS", "MURPHY USA",
]
...
async def load_and_score_async(conn) -> list[dict]:
    ...
    rows = await conn.fetch(
        "SELECT id, name, address, city, zip_code, phone, latitude, longitude FROM ga_retailers ORDER BY name"
    )
